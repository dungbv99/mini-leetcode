...
function NotificationButton() {
  ...
  const fetchNotification = () => {
    let fromId = null;
    const fetchedNotices = notifications.get();

    if (fetchedNotices) {
      fromId = fetchedNotices[fetchedNotices.length - 1].id;
    }
    ...
    request(
      ...
      (res) => {
        let data = res.data;
        const notices = processNotificationsContent(data.notifications.content);

        if (fromId === null) {
          notifications.set(notices);
        } else {
          notifications.merge(notices);
        }
